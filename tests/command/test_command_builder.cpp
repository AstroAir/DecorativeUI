...
private slots:
    void initTestCase() { qDebug() << "🧪 Starting Command Builder Tests"; }

    void cleanupTestCase() { qDebug() << "✅ Command Builder Tests completed"; }
    ...
    void testBasicCommandBuilder() {
        try {
            ...
            QCOMPARE(state->getProperty<QString>("text"),
                     QString("Test Button"));
            QCOMPARE(state->getProperty<bool>("enabled"), true);

        } catch (const std::exception& e) {
            QFAIL(QString("CommandBuilder test failed: %1")
                      .arg(e.what())
                      .toUtf8()
                      .constData());
        }
    ...
    void testCommandBuilderChaining() {
        try {
            ...
            auto state = command->getState();
            QCOMPARE(state->getProperty<QString>("placeholder"),
                     QString("Enter text..."));
            ...
            QFAIL(QString("CommandBuilder chaining test failed: %1")
                      .arg(e.what())
                      .toUtf8()
                      .constData());
        }
    ...
    void testCommandBuilderWithCustomProperties() {
        try {
            ...
            auto state = command->getState();
            QCOMPARE(state->getProperty<QString>("customProp1"),
                     QString("value1"));
            QCOMPARE(state->getProperty<int>("customProp2"), 42);
            QCOMPARE(state->getProperty<bool>("customProp3"), true);

        } catch (const std::exception& e) {
            QFAIL(QString("CommandBuilder custom properties test failed: %1")
                      .arg(e.what())
                      .toUtf8()
                      .constData());
        }
        ...
        try {
            ...
            auto command =
                UI::CommandBuilder("Button")
                    .text("Event Test")
                    .onClick([&eventHandled]() { eventHandled = true; })
                    .build();

            QVERIFY(command != nullptr);

            // Simulate click event (this would depend on the actual event
            // system implementation) For now, just verify the command was
            // created successfully
            QCOMPARE(command->getState()->getProperty<QString>("text"),
                     QString("Event Test"));

        } catch (const std::exception& e) {
            QFAIL(QString("CommandBuilder event handlers test failed: %1")
                      .arg(e.what())
                      .toUtf8()
                      .constData());
        }
        ...
        try {
            auto container =
                UI::CommandHierarchyBuilder("Container")
                    ...
                    .addChild("Button",
                              [](UI::CommandBuilder& button) {
                                  button.text("Child Button").enabled(true);
                              })
                    .build();
            ...
            if (children.size() >= 2) {
                ...
                QCOMPARE(children[0]->getState()->getProperty<QString>("text"),
                         QString("Child Label"));

                QCOMPARE(children[1]->getCommandType(), QString("Button"));
                QCOMPARE(children[1]->getState()->getProperty<QString>("text"),
                         QString("Child Button"));
                QCOMPARE(children[1]->getState()->getProperty<bool>("enabled"),
                         true);
            }

        } catch (const std::exception& e) {
            QFAIL(QString("CommandHierarchyBuilder test failed: %1")
                      .arg(e.what())
                      .toUtf8()
                      .constData());
        }
    ...
    void testCommandBuilderStateBinding() {
        try {
            ...
            QVERIFY(command != nullptr);
            QCOMPARE(command->getState()->getProperty<QString>("placeholder"),
                     QString("Bound Input"));

            // Note: Actual state binding testing would require the state
            // management system For now, just verify the command was created
            // successfully

        } catch (const std::exception& e) {
            QFAIL(QString("CommandBuilder state binding test failed: %1")
                      .arg(e.what())
                      .toUtf8()
                      .constData());
        }
    ...
    void testCommandBuilderValidation() {
        try {
            ...
            // Note: Actual validation testing would require the validation
            // system For now, just verify the command was created successfully

        } catch (const std::exception& e) {
            QFAIL(QString("CommandBuilder validation test failed: %1")
                      .arg(e.what())
                      .toUtf8()
                      .constData());
        }
    ...
    void testCommandBuilderStyling() {
        try {
            ...
            auto state = command->getState();
            QCOMPARE(state->getProperty<QString>("text"),
                     QString("Styled Label"));
            QCOMPARE(state->getProperty<QString>("style"),
                     QString("color: red; font-weight: bold;"));

        } catch (const std::exception& e) {
            QFAIL(QString("CommandBuilder styling test failed: %1")
                      .arg(e.what())
                      .toUtf8()
                      .constData());
        }
        ...
        try {
            auto mainContainer =
                UI::CommandHierarchyBuilder("Container")
                    ...
                    .addChild("Container",
                              [](UI::CommandBuilder& subContainer) {
                                  // For nested containers with children, we
                                  // need to use CommandHierarchyBuilder or add
                                  // children using the child() method on
                                  // CommandBuilder
                                  subContainer.layout("HBox")
                                      .spacing(5)
                                      .child(UI::CommandBuilder("Button").text(
                                          "Button 1"))
                                      .child(UI::CommandBuilder("Button").text(
                                          "Button 2"));
                              })
            ...
            if (children.size() >= 2) {
                ...
                if (subChildren.size() >= 2) {
                    QCOMPARE(subChildren[0]->getCommandType(),
                             QString("Button"));
                    QCOMPARE(subChildren[0]->getState()->getProperty<QString>(
                                 "text"),
                             QString("Button 1"));

                    QCOMPARE(subChildren[1]->getCommandType(),
                             QString("Button"));
                    QCOMPARE(subChildren[1]->getState()->getProperty<QString>(
                                 "text"),
                             QString("Button 2"));
                }
            }
            ...
            QFAIL(QString("Nested CommandHierarchy test failed: %1")
                      .arg(e.what())
                      .toUtf8()
                      .constData());
        }
...
private slots:
    void testCommandSystemDisabled() { QSKIP("Command system not enabled"); }
};
