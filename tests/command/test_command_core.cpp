...
private slots:
    void initTestCase() { qDebug() << "🧪 Starting Command Core Tests"; }

    void cleanupTestCase() { qDebug() << "✅ Command Core Tests completed"; }
    ...
    void testUICommandStateValidation() {
        ...
        // Set up validator
        state.setValidator(
            "count", [](const QVariant& value) { return value.toInt() >= 0; });
    ...
    void testUICommandStateSignals() {
        ...
        QSignalSpy propertyChangedSpy(&state,
                                      &UI::UICommandState::propertyChanged);
    ...
    void testUICommandStateDefaultValues() {
        ...
        QCOMPARE(state.getProperty<QString>("nonexistent", "default"),
                 QString("default"));
        ...
        QCOMPARE(state.getProperty<QString>("test", "default"),
                 QString("actual"));
    }
};
