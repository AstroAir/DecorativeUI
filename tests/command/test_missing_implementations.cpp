...
void TestMissingImplementations::testRegisterComponentCommands() {
    ...
    QVERIFY_EXCEPTION_THROWN(registerComponentCommands(), std::exception) == false;
...
void TestMissingImplementations::testStatusBarAddWidget() {
    ...
    QVERIFY(result.getValue().toString().contains("added to StatusBar successfully"));
}
...
void TestMissingImplementations::testStatusBarRemoveWidget() {
    ...
    QVERIFY(result.isSuccess());
    QVERIFY(result.getValue().toString().contains("removed from StatusBar successfully"));
}
...
void TestMissingImplementations::testComputedStateRecomputation() {
    ...
    QVERIFY_EXCEPTION_THROWN(stateManager.updateDependents("nonExistentKey"), std::exception) == false;
}
