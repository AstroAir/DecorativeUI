...
void PerformanceComprehensiveTest::cleanup() { test_widget_.reset(); }
...
void PerformanceComprehensiveTest::testWidgetCreationPerformance() {
    ...
    qDebug() << "Created" << num_widgets << "widgets in" << creation_time
             << "ms";
    qDebug() << "Destroyed" << num_widgets << "widgets in" << destruction_time
             << "ms";
    qDebug() << "Average creation time:" << (double)creation_time / num_widgets
             << "ms per widget";
    ...
    QVERIFY((double)creation_time / num_widgets <
            1.0);  // Less than 1ms per widget
}
...
void PerformanceComprehensiveTest::testLayoutPerformance() {
    ...
    qDebug() << "Added" << num_widgets << "widgets to layout in" << layout_time
             << "ms";
...
void PerformanceComprehensiveTest::testSignalSlotPerformance() {
    ...
    qDebug() << "Average signal time:" << (double)signal_time / num_signals
             << "ms per signal";

    QVERIFY(signal_count == num_signals);
    QVERIFY(signal_time < 1000);  // Should complete within 1 second
    QVERIFY((double)signal_time / num_signals <
            0.1);  // Less than 0.1ms per signal
}
...
void PerformanceComprehensiveTest::testPropertyAccessPerformance() {
    ...
    qDebug() << "Set property" << num_operations << "times in" << set_time
             << "ms";
    qDebug() << "Get property" << num_operations << "times in" << get_time
             << "ms";
...
void PerformanceComprehensiveTest::testMemoryAllocationPerformance() {
    ...
    qDebug() << "Allocated" << num_allocations << "blocks ("
             << (num_allocations * allocation_size / 1024) << "KB total) in"
             << allocation_time << "ms";
...
void PerformanceComprehensiveTest::testThreadCreationPerformance() {
    ...
    qDebug() << "Created and started" << num_threads << "threads in"
             << creation_time << "ms";
...
void PerformanceComprehensiveTest::testConcurrentOperations() {
    ...
    qDebug() << "Completed" << (num_threads * operations_per_thread)
             << "operations across" << num_threads << "threads in"
             << concurrent_time << "ms";
    qDebug() << "Operations per second:"
             << (double)(num_threads * operations_per_thread) * 1000 /
                    concurrent_time;
...
void PerformanceComprehensiveTest::testUIUpdatePerformance() {
    ...
    qDebug() << "Performed" << num_updates << "UI updates in" << update_time
             << "ms";
    qDebug() << "Average update time:" << (double)update_time / num_updates
             << "ms per update";

    // Performance assertions
    QVERIFY(update_time < 5000);  // Should update within 5 seconds
    QVERIFY((double)update_time / num_updates <
            5.0);  // Less than 5ms per update
}
...
void PerformanceComprehensiveTest::testStressWidgetCreation() {
    ...
    for (int i = 0; i < stress_widgets; ++i) {
        auto widget =
            std::make_unique<QLabel>(QString("Stress Widget %1").arg(i));
    ...
    qDebug() << "Created" << stress_widgets << "complex widgets in"
             << stress_time << "ms";
    qDebug() << "Memory usage (approximate):"
             << (stress_widgets * sizeof(QLabel)) / 1024 << "KB";
...
void PerformanceComprehensiveTest::testStressSignalEmission() {
    ...
    qDebug() << "Emitted" << stress_signals << "signals (with 5 slots each) in"
             << stress_signal_time << "ms";
    qDebug() << "Total slot executions:" << signal_counter.load();
    qDebug() << "Signals per second:"
             << (double)stress_signals * 1000 / stress_signal_time;
...
void PerformanceComprehensiveTest::testStressMemoryOperations() {
    ...
    qDebug() << "Performed" << stress_allocations
             << "variable-size allocations in" << stress_memory_time << "ms";
    qDebug() << "Final allocations remaining:" << stress_memory.size();
    qDebug() << "Allocations per second:"
             << (double)stress_allocations * 1000 / stress_memory_time;

    // Performance assertions
    QVERIFY(stress_memory_time < 5000);  // Should complete within 5 seconds
    QVERIFY(stress_memory.size() >
            0);  // Should have some allocations remaining
