...
private slots:
    ...
    void testButtonLabels() {
        ...
        QStringList expectedLabels = {"Start Profiling", "Stop Profiling", "Reset", "Export Profile"};
        ...
        for (const QString& expectedLabel : expectedLabels) {
            ...
            QVERIFY2(found, QString("Expected button label '%1' not found").arg(expectedLabel).toLocal8Bit());
        }
    }
    ...
    void testWidgetLayout() {
        ...
        QVERIFY(layout->count() >= 6);  // 4 buttons + progress bar + status label
    }
