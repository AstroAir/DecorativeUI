...
void AnimationSystemTest::cleanup() { testWidget.reset(); }
...
void AnimationSystemTest::testAnimationConfig() {
    ...
    AnimationConfig customConfig{.duration = 500ms,
...
void AnimationSystemTest::testAnimationBuilderEventHandlers() {
    ...
    auto& finishedRef =
        builder.onFinished([&finishedCalled]() { finishedCalled = true; });
    QCOMPARE(&finishedRef, &builder);

    auto& startedRef =
        builder.onStarted([&startedCalled]() { startedCalled = true; });
...
void AnimationSystemTest::testAnimationBuilderBuild() {
    ...
    auto animation = builder.animate("windowOpacity", 0.0, 1.0)
                         .duration(200ms)
...
void AnimationSystemTest::testSlideInPreset() {
    ...
    auto slideBuilder =
        Presets::slideIn(testWidget.get(), fromPos, toPos, 400ms);
...
void AnimationSystemTest::testAnimationSequenceBuild() {
    ...
    auto builtSequence =
        sequence.then(fadeIn).pause(50ms).then(fadeOut).build();
...
void AnimationSystemTest::testAnimateUtility() {
    ...
    auto animation =
        builder.animate("windowOpacity", 0.5, 1.0).duration(150ms).build();
...
void AnimationSystemTest::testParallelUtility() {
    ...
    auto slideAnimation =
        Presets::slideIn(testWidget.get(), QPoint(0, 0), QPoint(50, 50), 200ms)
            .build();

    auto parallelGroup =
        parallel(fadeAnimation.release(), slideAnimation.release());
...
void AnimationSystemTest::testAnimationExecution() {
    ...
    auto animation =
        animate(testWidget.get())
            .animate("windowOpacity", 1.0, 0.5)
            .duration(50ms)  // Short duration for test
            .onFinished([&animationFinished]() { animationFinished = true; })
            .build();
