...
private slots:
    ...
    void testJSONParserMaxDepth() {
        ...
        try {
            ...
            QVERIFY(e.getMessage().find("depth") != std::string::npos || e.getMessage().find("nested") != std::string::npos);
        }
    }
    ...
    void testComponentRegistryBasicRegistration() {
        ...
        registry.registerComponent<QPushButton>("Button", [](const QJsonObject& config) -> std::unique_ptr<QPushButton> {
    ...
    void testComponentRegistryComponentCreation() {
        ...
        registry.registerComponent<QLabel>("Label", [](const QJsonObject& config) -> std::unique_ptr<QLabel> {
    ...
    void testComponentRegistryUnregistration() {
        ...
        registry.registerComponent<QWidget>("TempComponent", [](const QJsonObject&) -> std::unique_ptr<QWidget> {
    ...
    void testComponentRegistryErrorHandling() {
        ...
        registry.registerComponent<QWidget>("NullFactory", [](const QJsonObject& config) -> std::unique_ptr<QWidget> {
            return nullptr;
        });
    ...
    void testJSONValidatorSchemaValidation() {
        ...
        auto invalid_doc = QJsonDocument::fromJson(invalid_schema_json.toUtf8(), &error);
        QVERIFY(error.error == QJsonParseError::NoError);
    ...
    void testJSONValidatorErrorRecovery() {
        ...
        QVERIFY(!is_valid || is_valid); // Either result is acceptable for empty object
    }
