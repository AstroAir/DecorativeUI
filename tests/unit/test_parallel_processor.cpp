...
TEST_F(ParallelProcessorTest, BasicTaskSubmission) {
    ...
    QString taskId = processor.submitTask("test_task", TaskPriority::Normal, 
                                        ExecutionContext::ThreadPool, [&taskExecuted]() {
        taskExecuted = true;
    });
...
TEST_F(ParallelProcessorTest, TaskCancellation) {
    ...
    QString taskId = processor.submitTask("cancel_test", TaskPriority::Normal, 
                                        ExecutionContext::ThreadPool, []() {
...
TEST_F(ParallelProcessorTest, FileProcessorProcessing) {
    ...
    auto processFuture = fileProcessor.processFilesAsync(testFiles, 
...
TEST_F(ParallelProcessorTest, PropertyBinderBasic) {
    ...
    auto bindFuture = propertyBinder.bindPropertyAsync<int>(&testObject, "testProperty", 
        []() -> int {
            return 100;
        });
