...
private slots:
    void runPerformanceTest()
    {
        ...
        for (int i = 0; i < 1000; ++i) {
            ...
            if (i % 100 == 0) {
                ...
                m_log->append(QString("Completed %1 operations in %2ms").arg(i).arg(elapsed));
            }
        }
        
        qint64 totalTime = timer.elapsed();
        m_log->append(QString("Performance test completed in %1ms").arg(totalTime));
        m_log->append(QString("Average time per operation: %1ms").arg(totalTime / 1000.0));
    }
    ...
    void updatePerformanceMetrics()
    {
        ...
        m_fpsLabel->setText(QString("Update interval: %1ms").arg(duration.count()));
        lastUpdate = now;
    }
...
private:
    void setupUI()
    {
        ...
        connect(testButton, &QPushButton::clicked, this, &PerformanceWidget::runPerformanceTest);
        controlsLayout->addWidget(testButton);
        
        auto clearButton = new QPushButton("Clear Log");
        connect(clearButton, &QPushButton::clicked, [this]() { m_log->clear(); });
        ...
        tips->setStyleSheet("background-color: #f0f0f0; padding: 10px; border-radius: 5px;");
    ...
    void setupPerformanceMonitoring()
    {
        ...
        connect(timer, &QTimer::timeout, this, &PerformanceWidget::updatePerformanceMetrics);
        timer->start(100);
    }
