...
private slots:
    ...
    void onReloadFailed(const QString& file_path, const QString& error) {
        qCritical() << "❌ Hot reload failed for:" << file_path
                    << "Error:" << error;
        ...
        if (main_widget_) {
            QMessageBox::warning(
                main_widget_.get(), "Hot Reload Failed",
    ...
    void onTestButtonClicked() {
        QMessageBox::information(main_widget_.get(), "Hot Reload Test",
                                 "🎉 This button works!\n\n"
        ...
        if (hot_reload_manager_) {
            ...
            if (status_label_) {
                status_label_->setText(
                    QString("%1 Hot reload %2").arg(emoji, status));
                status_label_->setStyleSheet(enabled ? "color: #e74c3c;"
                                                     : "color: #27ae60;");
            }
        }
    }
...
private:
    ...
    void setupUILoader() {
        ...
        ui_loader_->registerEventHandler("testButton",
                                         [this]() { onTestButtonClicked(); });

        ui_loader_->registerEventHandler(
            "toggleHotReload", [this]() { onToggleHotReloadClicked(); });

        ui_loader_->registerEventHandler("manualReload",
                                         [this]() { onManualReloadClicked(); });
    ...
    void createUI() {
        try {
            ...
            main_widget_->setWindowTitle(
                "04 - Basic Hot Reload | DeclarativeUI");
    ...
    void setupHotReload() {
        try {
            ...
            connect(hot_reload_manager_.get(),
                    ...
                    &BasicHotReloadApp::onReloadStarted);

            connect(hot_reload_manager_.get(),
                    &HotReload::HotReloadManager::reloadCompleted, this,
                    &BasicHotReloadApp::onReloadCompleted);

            connect(hot_reload_manager_.get(),
                    &HotReload::HotReloadManager::reloadFailed, this,
                    &BasicHotReloadApp::onReloadFailed);

            // Register UI file for hot reloading
            QString ui_file = "resources/hot_reload_ui.json";
            if (QFileInfo::exists(ui_file)) {
                hot_reload_manager_->registerUIFile(ui_file,
                                                    main_widget_.get());
                ...
                qWarning() << "Cannot enable hot reload - UI file not found:"
                           << ui_file;
            }
    ...
    std::unique_ptr<QWidget> createFallbackUI() {
        ...
        widget->setWindowTitle(
            "04 - Basic Hot Reload (Fallback) | DeclarativeUI");
        ...
        test_button->setStyleSheet(
            ...
            "padding: 10px 20px; border: none; border-radius: 5px; font-size: "
            "14px; }"
            "QPushButton:hover { background-color: #2980b9; }");
        connect(test_button, &QPushButton::clicked, this,
                &BasicHotReloadApp::onTestButtonClicked);
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        qDebug() << "";
        qDebug() << "🔥 Try editing 'resources/hot_reload_ui.json' to see hot "
                    "reload in action!";
