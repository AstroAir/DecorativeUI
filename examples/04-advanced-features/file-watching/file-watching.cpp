...
private slots:
    ...
    void onReloadStarted(const QString& file_path) {
        ...
        auto progress =
            main_widget_->findChild<QProgressBar*>("reloadProgress");
        if (progress) {
    ...
    void onReloadCompleted(const QString& file_path) {
        ...
        logEvent("RELOAD_COMPLETE", file_path,
                 QString("Completed in %1ms").arg(elapsed));
        ...
        if (progress) {
            ...
            QTimer::singleShot(1000,
                               [progress]() { progress->setVisible(false); });
        }
    }
    ...
    void onPerformanceTestClicked() {
        ...
        for (int i = 0; i < 10; ++i) {
            QTimer::singleShot(i * 100, [this, i]() {
                QString test_file =
                    QString("resources/test_files/test_%1.json").arg(i);
                ...
                logEvent("PERF_TEST", test_file,
                         QString("Test file %1 created").arg(i));
            });
        }
        ...
        QTimer::singleShot(1500, [this, test_timer]() {
            ...
            logEvent("PERF_TEST", "complete",
                     QString("Test completed in %1ms").arg(total_time));
        });
    }
...
private:
    ...
    void setupUILoader() {
        ...
        ui_loader_->registerEventHandler("addWatchPath",
                                         [this]() { onAddWatchPathClicked(); });

        ui_loader_->registerEventHandler(
            "removeWatchPath", [this]() { onRemoveWatchPathClicked(); });

        ui_loader_->registerEventHandler(
            "configureWatcher", [this]() { onConfigureWatcherClicked(); });

        ui_loader_->registerEventHandler("testReload",
                                         [this]() { onTestReloadClicked(); });

        ui_loader_->registerEventHandler("clearLog",
                                         [this]() { onClearLogClicked(); });

        ui_loader_->registerEventHandler(
            "performanceTest", [this]() { onPerformanceTestClicked(); });
            ...
            if (QFileInfo::exists(ui_file)) {
                ...
                if (main_widget_) {
                    main_widget_->setWindowTitle(
                        "21 - File Watching | DeclarativeUI");
                    connectUIEvents();
                    updateWatchedFilesList();
                    updatePerformanceDisplay();
    ...
    void connectUIEvents() {
        ...
        auto filters_combo =
            main_widget_->findChild<QComboBox*>("filtersCombo");
        if (filters_combo) {
            connect(filters_combo, &QComboBox::currentTextChanged, this,
                    &FileWatchingApp::onConfigureWatcherClicked);
        }
    ...
    void setupAdvancedHotReload() {
        try {
            ...
            connect(hot_reload_manager_.get(),
                    &HotReload::HotReloadManager::reloadStarted, this,
                    &FileWatchingApp::onReloadStarted);

            connect(hot_reload_manager_.get(),
                    &HotReload::HotReloadManager::reloadCompleted, this,
                    &FileWatchingApp::onReloadCompleted);

            connect(hot_reload_manager_.get(),
                    &HotReload::HotReloadManager::reloadFailed, this,
                    &FileWatchingApp::onReloadFailed);
            ...
            QStringList watch_paths = {"resources/", "resources/test_files/"};
    ...
    void startMonitoring() {
        ...
        connect(monitoring_timer_, &QTimer::timeout,
    ...
    void updateWatchedFilesList() {
        auto file_list =
            main_widget_->findChild<QListWidget*>("watchedFilesList");
        if (!file_list)
            return;
        ...
        QStringList watched_paths = {"resources/", "resources/test_files/",
    ...
    void updatePerformanceDisplay() {
        ...
        auto perf_display =
            main_widget_->findChild<QLabel*>("performanceDisplay");
        if (perf_display) {
            QString perf_text =
                QString(
                    "📊 Performance Metrics:\n"
                    ...
                    "• Memory usage: %4MB")
                    .arg(QString::number(
                             performance_monitor_->getAverageMetrics()
                                 .total_time_ms,
                             'f', 1),
                         QString::number(
                             performance_monitor_->getAverageMetrics()
                                 .success_count),
                         QString::number(
                             performance_monitor_->getSuccessRate() * 100.0,
                             'f', 1),
                         QString::number(50.0, 'f',
                                         1)  // Placeholder for memory usage
                    );
    ...
    void logEvent(const QString& type, const QString& path,
        ...
        if (log_display) {
            ...
            QString log_entry = QString("[%1] %2 | %3 | %4")
                                    .arg(timestamp, type, path, details);
