...
#include "Command/SpecializedCommands.hpp"
 ...
 * 
 * This example shows how to build a complete UI application using the Command system
 * with proper MVC separation, state management, and event handling.
 */
...
public:
    static std::shared_ptr<BaseUICommand> create() {
        ...
        return CommandHierarchyBuilder("Container")
            ...
            // Name field
            .addContainer("Container", [](CommandHierarchyBuilder& row) {
                row.layout("HBox")
                   .addChild("Label", [](CommandBuilder& label) {
            ...
            .addContainer("Container", [](CommandHierarchyBuilder& row) {
                row.layout("HBox")
                   ...
                   .addChild("SpinBox", [](CommandBuilder& spinbox) {
            ...
            // Buttons
            .addContainer("Container", [](CommandHierarchyBuilder& buttons) {
                buttons.layout("HBox")
                       .spacing(10)
...
public:
    static std::shared_ptr<BaseUICommand> create() {
        ...
        return CommandHierarchyBuilder("Container")
            ...
            // Counter controls
            .addContainer("Container", [](CommandHierarchyBuilder& controls) {
                controls.layout("HBox")
                        .spacing(10)
                        .addChild("Button", [](CommandBuilder& decrement) {
                            decrement.text("-")
                                    .onClick([]() {
                                        // Decrement counter in state
                                        auto& state = DeclarativeUI::Binding::StateManager::instance();
                                        // Implementation would decrement the counter
                                        qDebug() << "Decrement counter";
                                    });
                        })
                        .addChild("Slider", [](CommandBuilder& slider) {
            ...
            .addChild("ProgressBar", [](CommandBuilder& progress) {
...
public:
    static std::shared_ptr<BaseUICommand> create() {
        ...
        auto container = CommandHierarchyBuilder("Container")
            ...
            .addChild("CheckBox", [](CommandBuilder& checkbox) {
                checkbox.text("Check me for state events")
                        .onEvent("stateChanged", [](const QVariant& state) {
                            qDebug() << "Checkbox state changed:" << state.toInt();
                        });
            })
        ...
        CommandEventDispatcher::instance().registerHandler(
            ...
            [](const CommandEvent& event) {
                qDebug() << "Low priority click handler, event ID:" << event.getId().toString();
            },
...
public:
    explicit CustomColorPickerCommand(QObject* parent = nullptr) : BaseUICommand(parent) {
    ...
    UICommandMetadata getMetadata() const override {
        UICommandMetadata metadata("ColorPicker", "QPushButton", "Color Picker", "A custom color picker command");
        metadata.supported_events = {"clicked", "colorChanged"};
        metadata.default_properties = QJsonObject{
            {"color", QColor(Qt::white).name()},
            {"text", "Pick Color"}
        };
        return metadata;
    }
