...
public:
    StateIntegrationExampleWindow(QWidget* parent = nullptr)
        : QMainWindow(parent) {
        ...
        try {
            ...
            auto counterButton =
                UI::CommandBuilder("Button")
                    ...
                    .onClick([&stateAdapter]() {
                        ...
                        auto counterState =
                            stateManager.getState<int>("counter");
                        if (counterState) {
                ...
                UI::CommandBuilder("Button")
                    ...
                    .onClick([]() {
                        auto& stateManager =
                            DeclarativeUI::Binding::StateManager::instance();
                        auto enabledState =
                            stateManager.getState<bool>("enabled");
                        if (enabledState) {
            ...
            auto mainContainer =
                UI::CommandBuilder("Container")
                    ...
                    .child(toggleButton)
                    .child(UI::CommandBuilder("Label")
                               .text("All components are synchronized through "
                                     "the state manager")
            ...
            auto widget =
                UI::WidgetMapper::instance().createWidget(mainContainer.get());
            if (widget) {
                setCentralWidget(widget.release());
                qDebug()
                    << "✅ State integration example UI created successfully";
            ...
            qWarning() << "❌ Error creating state integration example:"
                       << e.what();
        }
    }
};
