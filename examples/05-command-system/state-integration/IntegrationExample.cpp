            ...
            CommandHierarchyBuilder("Container")
                ...
                .addChild("Label",
                          [](CommandBuilder& label) {
                              label
                                  .text(
                                      "Integration Example - Legacy and "
                                      "Command Systems")
                ...
                .addContainer("Container",
                              [this](CommandHierarchyBuilder& section) {
                                  section.layout("VBox").spacing(10).addChild(
                                      "Label", [](CommandBuilder& label) {
                    ...
                    [this](CommandHierarchyBuilder& section) {
                        section.layout("VBox")
                            ...
                            .addChild("Button", [this](CommandBuilder& button) {
                                button.text("Command Button").onClick([this]() {
...
void IntegrationExample::demonstrateComponentToCommandConversion() {
    ...
    try {
        ...
        if (component_adapter_) {
            auto commandButton =
                component_adapter_->convertToCommand(legacy_button_.get());
...
void IntegrationExample::demonstrateCommandToComponentConversion() {
    ...
    try {
        // Create command
        auto commandButton =
            CommandBuilder("Button").text("Command Button").build();
...
void IntegrationExample::demonstrateBidirectionalSync() {
    ...
    try {
        ...
        if (state_adapter_) {
            ...
            state_adapter_->bindCommand(sync_command_button_, SYNC_STATE,
                                        "text");

            // Demonstrate sync
            state_adapter_->setCommandState(sync_command_button_, "text",
...
void IntegrationExample::demonstrateHybridContainer() {
    ...
    try {
        if (component_adapter_) {
            ...
            // Implementation would create a container with both legacy and
            // command components
...
void IntegrationExample::onCommandButtonClicked() {
    ...
    auto& stateManager = DeclarativeUI::Binding::StateManager::instance();
    stateManager.setState(MESSAGE_STATE,
                          QString("Command button was clicked!"));
}
...
void IntegrationExample::onStateChanged(const QString& key,
                                        const QVariant& value) {
    qDebug() << "🔄 State changed in integration example:" << key << "="
             << value;
}
