...
int main(int argc, char* argv[]) {
    ...
    radio1->text("Fast").checked(true).onToggled([](bool checked) {
        if (checked)
            qDebug() << "Fast mode selected";
    });

    auto* radio2 = new RadioButton();
    radio2->text("Medium").onToggled([](bool checked) {
        if (checked)
            qDebug() << "Medium mode selected";
    });

    auto* radio3 = new RadioButton();
    radio3->text("Slow").onToggled([](bool checked) {
        if (checked)
            qDebug() << "Slow mode selected";
    });
    ...
    dial->minimum(0)
        ...
        .wrapping(true)
        .onValueChanged(
            [](int value) { qDebug() << "Dial value changed to:" << value; });
    dial->initialize();
    ...
    calendar->gridVisible(true)
        ...
        .onSelectionChanged([]() { qDebug() << "Calendar selection changed"; });
    calendar->initialize();
    ...
    splitter->orientation(Qt::Horizontal)
        ...
        .onSplitterMoved([](int pos, int index) {
            qDebug() << "Splitter moved - Position:" << pos
                     << "Index:" << index;
        });
    splitter->initialize();
    ...
    splitter->addWidget(leftPanel).addWidget(rightPanel).setSizes({200, 300});
    ...
    scrollContent->setStyleSheet(
        "background-color: #FFF3E0; border: 2px dashed #FF9800;");
    ...
    QObject::connect(qobject_cast<QDial*>(dial->getWidget()),
                     &QDial::valueChanged,
                     [lcd](int value) { lcd->setValue(value); });
