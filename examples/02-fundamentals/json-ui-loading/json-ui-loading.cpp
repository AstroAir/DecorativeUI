...
private slots:
    void onGreetClicked() {
        ...
        QMessageBox::information(
            main_widget_.get(), "JSON UI Event",
    ...
    void onShowInfoClicked() {
        QMessageBox::information(main_widget_.get(), "JSON UI Info",
                                 "<h3>JSON UI Loading Example</h3>"
                                 "<p>This UI was loaded from: "
                                 "<code>resources/basic_ui.json</code></p>"
    ...
    void onReloadUIClicked() {
        ...
        try {
            auto new_widget =
                ui_loader_->loadFromFile("resources/basic_ui.json");
            if (new_widget) {
            ...
            QMessageBox::warning(
                main_widget_.get(), "Reload Failed",
    ...
    void onCommandGreetClicked() {
        ...
        QString message = QString(
                              "🎉 Hello, %1!\n\nThis button was created using "
                              "the Command system!")
                              .arg(name);
        stateManager.setState("app.message", message);

        QMessageBox::information(main_widget_.get(), "Command System Event",
                                 message);
    ...
    void onCommandClearClicked() {
        ...
        stateManager.setState("app.message",
                              QString("All fields cleared via Command system"));

        QMessageBox::information(
            main_widget_.get(), "Command System",
    ...
    void onCommandShowInfoClicked() {
        QMessageBox::information(main_widget_.get(), "Command System Info",
...
private:
    ...
    void setupUILoader() {
        ...
        ui_loader_->registerEventHandler("greetUser",
                                         [this]() { onGreetClicked(); });

        ui_loader_->registerEventHandler("clearFields",
                                         [this]() { onClearClicked(); });

        ui_loader_->registerEventHandler("showInfo",
                                         [this]() { onShowInfoClicked(); });

        ui_loader_->registerEventHandler("reloadUI",
                                         [this]() { onReloadUIClicked(); });

#ifdef DECLARATIVE_UI_COMMAND_SYSTEM_ENABLED
#ifdef DECLARATIVE_UI_ADAPTERS_ENABLED
        // Set up Command system JSON loader
        command_loader_ =
            std::make_unique<Command::Adapters::JSONCommandLoader>();
        ...
        command_loader_->registerEventHandler(
            "greetUserCommand",
            [this](const QVariant&) { onCommandGreetClicked(); });

        command_loader_->registerEventHandler(
            "clearFieldsCommand",
            [this](const QVariant&) { onCommandClearClicked(); });

        command_loader_->registerEventHandler(
            "showInfoCommand",
            [this](const QVariant&) { onCommandShowInfoClicked(); });

        // Initialize state manager
        auto& stateManager = Binding::StateManager::instance();
        stateManager.setState("app.title",
                              QString("JSON UI Loading with Commands"));
        stateManager.setState("user.name", QString(""));
        stateManager.setState("app.message",
                              QString("Welcome to enhanced JSON loading!"));
    ...
    void createUI() {
        try {
            ...
            main_widget_->setWindowTitle(
                "02 - JSON UI Loading | DeclarativeUI");
    ...
    std::unique_ptr<QWidget> createFallbackUI() {
        ...
        widget->setWindowTitle(
            "02 - JSON UI Loading (Fallback) | DeclarativeUI");
        ...
        connect(greet_button, &QPushButton::clicked, this,
                &JSONUIApp::onGreetClicked);
        connect(clear_button, &QPushButton::clicked, this,
                &JSONUIApp::onClearClicked);
        connect(info_button, &QPushButton::clicked, this,
                &JSONUIApp::onShowInfoClicked);
...
#else
        qDebug() << "   - Command system JSON loading (build with "
                    "BUILD_ADAPTERS=ON)";
#endif
#else
        qDebug() << "   - Command system JSON loading (build with "
