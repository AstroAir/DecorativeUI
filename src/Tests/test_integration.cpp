...
private slots:
        ...
        auto main_widget =
            create<QWidget>()
                ...
                .child<QWidget>([](auto& container) {
                    ...
                    button->text("Integrated Button").onClick([]() {
    ...
    void testStateManagementWithCommands() {
        ...
        auto message_state = state_manager.createState<QString>(
            "message", QString("Initial message"));
        ...
        auto result =
            command_manager.getInvoker().execute("update_state", context);
        ...
        auto string_result = command_manager.getInvoker().execute(
            "update_state", string_context);
    ...
    void testJSONUIWithHotReload() {
        ...
        QTemporaryFile ui_file(temp_dir_->path() +
        ...
        QSignalSpy reload_spy(&hot_reload_manager,
        ...
        // Verify reload occurred (signal count may be 0 if file watching is not
        // available)
    ...
    void testCommandIntegrationWithComponents() {
        ...
        clipboard_context.setParameter("text",
                                       QString("Integration test text"));
    ...
    void testFullApplicationWorkflow() {
        ...
        auto app_title_state = state_manager.createState<QString>(
            "app_title", QString("Integration Demo"));
        auto user_input_state =
            state_manager.createState<QString>("user_input", QString(""));
        auto counter_state = state_manager.createState<int>("counter", 0);
        ...
        auto* inc_widget =
            qobject_cast<QPushButton*>(increment_button.getWidget());
        auto* reset_widget =
            qobject_cast<QPushButton*>(reset_button.getWidget());
        ...
        try {
            ...
            auto result = command_manager.getInvoker().execute(
                "nonexistent_command", invalid_context);
    ...
    void testPerformanceIntegration() {
        ...
        for (int i = 0; i < 50; ++i) {
            // Create button
            auto button = std::make_unique<Button>();
            button->text(QString("Button %1").arg(i)).onClick([i]() {
            ...
            auto result =
                command_manager.getInvoker().execute("clipboard.copy", context);
            QVERIFY(result.isSuccess());
        }

        qint64 elapsed = timer.elapsed();
        qDebug() << "Created 50 components and executed 50 commands in"
                 << elapsed << "ms";
        ...
        for (size_t i = 0; i < buttons.size(); ++i) {
            ...
            QVERIFY(state_manager.getState<int>(QString("item_%1").arg(i)) !=
                    nullptr);
        }
    }
    ...
    void testErrorRecoveryIntegration() {
        ...
        try {
            auto invalid_state =
                state_manager.getState<QString>("nonexistent_key");
        ...
        CommandContext invalid_context;
        auto result = command_manager.getInvoker().execute("invalid_command",
                                                           invalid_context);
        QVERIFY(result.isError());
        ...
        auto recovery_state = state_manager.createState<QString>(
            "recovery_test", QString("System recovered"));
        ...
        auto valid_result = command_manager.getInvoker().execute(
            "clipboard.copy", valid_context);
