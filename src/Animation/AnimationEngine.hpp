...
#include <QTimer>
#include <QWidget>
// #include <QOpenGLWidget>  // Commented out - may not be available in this Qt
// installation
...
// **Animation state**
enum class AnimationState { Stopped, Running, Paused, Finished };
...
public:
    void addKeyframe(double time_ratio, const T& value,
                     EasingType easing = EasingType::Linear) {
    ...
    T interpolate(double time_ratio) const {
        ...
        auto it =
            std::upper_bound(keyframes_.begin(), keyframes_.end(), time_ratio,
...
struct AnimationTarget {
    ...
    bool isValid() const {
        return object != nullptr && !property_name.isEmpty();
    }
};
...
public:
    ...
    std::shared_ptr<AnimationGroup> createAnimationGroup(
        AnimationGroup::GroupType type);

    // **Convenience methods**
    std::shared_ptr<Animation> animateProperty(
        QObject* object, const QString& property, const QVariant& start_value,
        const QVariant& end_value, int duration_ms = 1000,
        EasingType easing = EasingType::Linear);
    ...
    std::shared_ptr<Animation> slideIn(QWidget* widget,
                                       const QString& direction,
                                       int duration_ms = 500);
    std::shared_ptr<Animation> slideOut(QWidget* widget,
                                        const QString& direction,
                                        int duration_ms = 500);
    std::shared_ptr<Animation> scaleAnimation(QWidget* widget,
                                              double from_scale,
                                              double to_scale,
                                              int duration_ms = 500);
...
private:
    ...
    // std::unique_ptr<GPUAnimationRenderer> gpu_renderer_;  // Commented out
    // due to missing QOpenGLWidget
