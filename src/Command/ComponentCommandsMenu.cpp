...
CommandResult<QVariant> MenuBarCommand::execute(const CommandContext& context) {
    ...
    if (!menuBar) {
        return CommandResult<QVariant>(
            QString("MenuBar '%1' not found").arg(widget_name));
    }
...
CommandMetadata MenuBarCommand::getMetadata() const {
    return CommandMetadata("MenuBarCommand",
                           "Specialized command for MenuBar components");
}
...
CommandResult<QVariant> MenuBarCommand::validateRequiredParameter(
...
CommandResult<QVariant> MenuBarCommand::handleAddMenu(
    const CommandContext& context, QMenuBar* menuBar) {
    auto title = context.getParameter<QString>("title");
    if (!context.hasParameter("title")) {
        return CommandResult<QVariant>(
            QString("Missing title parameter for addMenu operation"));
    }

    [[maybe_unused]] auto menu = menuBar->addMenu(title);
    return CommandResult<QVariant>(QString("MenuBar menu added successfully"));
}

CommandResult<QVariant> MenuBarCommand::handleRemoveMenu(
    const CommandContext& context, QMenuBar* menuBar) {
    auto title = context.getParameter<QString>("title");
    if (!context.hasParameter("title")) {
        return CommandResult<QVariant>(
            QString("Missing title parameter for removeMenu operation"));
    }
    ...
    for (auto* action : menuBar->actions()) {
        if (action->text() == title) {
            ...
            return CommandResult<QVariant>(
                QString("MenuBar menu removed successfully"));
        }
    }
    return CommandResult<QVariant>(
        QString("Menu with title '%1' not found").arg(title));
}

CommandResult<QVariant> MenuBarCommand::handleSetNativeMenuBar(
    const CommandContext& context, QMenuBar* menuBar) {
    auto native = context.getParameter<bool>("native");
    if (!context.hasParameter("native")) {
        return CommandResult<QVariant>(
            QString("Missing native parameter for setNativeMenuBar operation"));
    }

    menuBar->setNativeMenuBar(native);
    return CommandResult<QVariant>(
        QString("MenuBar native menu bar set successfully"));
}
...
CommandResult<QVariant> StatusBarCommand::execute(
    ...
    if (!statusBar) {
        return CommandResult<QVariant>(
            QString("StatusBar '%1' not found").arg(widget_name));
    }
...
CommandMetadata StatusBarCommand::getMetadata() const {
    return CommandMetadata("StatusBarCommand",
                           "Specialized command for StatusBar components");
}
...
QWidget* StatusBarCommand::findWidgetByName(const QString& name,
                                            QWidget* parent) {
    for (auto* widget : QApplication::allWidgets()) {
...
CommandResult<QVariant> StatusBarCommand::validateRequiredParameter(
    const CommandContext& context, const QString& paramName) {
    if (!context.hasParameter(paramName)) {
        return CommandResult<QVariant>(
            QString("Missing required parameter: %1").arg(paramName));
...
CommandResult<QVariant> StatusBarCommand::handleShowMessage(
    const CommandContext& context, QStatusBar* statusBar) {
    auto message = context.getParameter<QString>("message");
    if (!context.hasParameter("message")) {
        return CommandResult<QVariant>(
            QString("Missing message parameter for showMessage operation"));
    }
    ...
    return CommandResult<QVariant>(
        QString("StatusBar message shown successfully"));
}

CommandResult<QVariant> StatusBarCommand::handleClearMessage(
    const CommandContext& context, QStatusBar* statusBar) {
    Q_UNUSED(context)  // clearMessage doesn't need additional parameters
    statusBar->clearMessage();
    return CommandResult<QVariant>(
        QString("StatusBar message cleared successfully"));
}

CommandResult<QVariant> StatusBarCommand::handleAddWidget(
    const CommandContext& context, QStatusBar* statusBar) {
    auto widgetName = context.getParameter<QString>("widgetName");
    if (!context.hasParameter("widgetName")) {
        return CommandResult<QVariant>(
            QString("Missing widgetName parameter for addWidget operation"));
    }
    ...
    if (!widgetToAdd) {
        return CommandResult<QVariant>(
            QString("Widget '%1' not found for addWidget operation")
                .arg(widgetName));
    }
    ...
    return CommandResult<QVariant>(
        QString("Widget '%1' added to StatusBar successfully").arg(widgetName));
}

CommandResult<QVariant> StatusBarCommand::handleRemoveWidget(
    const CommandContext& context, QStatusBar* statusBar) {
    auto widgetName = context.getParameter<QString>("widgetName");
    if (!context.hasParameter("widgetName")) {
        return CommandResult<QVariant>(
            QString("Missing widgetName parameter for removeWidget operation"));
    }
    ...
    if (!widgetToRemove) {
        return CommandResult<QVariant>(
            QString(
                "Widget '%1' not found in StatusBar for removeWidget operation")
                .arg(widgetName));
    }

    statusBar->removeWidget(widgetToRemove);
    return CommandResult<QVariant>(
        QString("Widget '%1' removed from StatusBar successfully")
            .arg(widgetName));
}
