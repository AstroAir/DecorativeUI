...
/**
 * @brief Comprehensive example demonstrating integration between Command system and existing components
 * 
...
public:
    ...
    static std::shared_ptr<UI::BaseUICommand> createHybridUI() {
        ...
        // Convert to Commands
        auto buttonCommand = ComponentSystemAdapter::instance().convertToCommand(legacyButton.get());
        ...
        auto container = UI::CommandBuilder("Container")
            ...
            .child(UI::CommandBuilder("TextInput").placeholder("Hybrid Input").bindToState("hybrid.input"))
            ...
            .addChild("TextInput", [](UI::CommandBuilder& input) {
                input.placeholder("Command Input")
                     ...
                     .onTextChanged([](const QString& text) {
                         ...
                         Adapters::State::set(nullptr, "command.label", QString("Input: %1").arg(text));
                     });
            })
    ...
    static void demonstrateMigrationSteps() {
        ...
        qDebug() << "Legacy UI created with" << legacyUI->children().size() << "children";
        
        // Step 2: Create hybrid UI
        qDebug() << "Step 2: Creating hybrid UI...";
        auto hybridUI = createHybridUI();
        qDebug() << "Hybrid UI created with command type:" << hybridUI->getCommandType();
        
        // Step 3: Create full command UI
        qDebug() << "Step 3: Creating command UI...";
        auto commandUI = createCommandUI();
        qDebug() << "Command UI created with" << commandUI->getChildren().size() << "children";
        
        // Demonstrate conversion
        qDebug() << "Step 4: Converting legacy to command...";
        auto& adapter = Adapters::ComponentSystemAdapter::instance();
        auto convertedCommands = adapter.convertComponentHierarchy(legacyUI.get());
        qDebug() << "Converted" << convertedCommands.size() << "components to commands";
    }
};
...
public:
    ...
    static std::shared_ptr<DeclarativeUI::Command::UI::BaseUICommand> loadMixedUI() {
        ...
        loader.registerEventHandler("onLegacyButtonClicked", [](const QVariant&) {
            qDebug() << "Legacy button clicked through JSON!";
        });
        
        loader.registerEventHandler("onCommandButtonClicked", [](const QVariant&) {
...
public:
    static void demonstrateStateIntegration() {
        ...
        auto legacyButton = std::make_unique<DeclarativeUI::Components::Button>();
        legacyButton->text("Legacy Button");
        
        auto commandButton = DeclarativeUI::Command::UI::CommandBuilder("Button")
            .text("Command Button")
            .build();
        
        // Get state adapter
        auto& stateAdapter = DeclarativeUI::Command::Adapters::CommandStateManagerAdapter::instance();
        ...
        auto adaptedLegacyButton = ComponentSystemAdapter::instance().convertToCommand(legacyButton.get());
        stateAdapter.bindCommand(adaptedLegacyButton, "shared.button.text", "text");

        // Demonstrate state synchronization
        qDebug() << "Setting shared state...";
        stateAdapter.setCommandState(commandButton, "text", QString("Synchronized Text"));

        // Both buttons should now have the same text
        qDebug() << "Command button text:" << commandButton->getState()->getProperty<QString>("text");
        qDebug() << "Adapted legacy button text:" << adaptedLegacyButton->getState()->getProperty<QString>("text");
        
        // Demonstrate batch updates
        qDebug() << "Demonstrating batch updates...";
        stateAdapter.beginBatchUpdate();
        stateAdapter.setCommandState(commandButton, "text", QString("Batch Update 1"));
...
public:
    static void comparePerformance() {
        ...
        for (int i = 0; i < iterations; ++i) {
            auto legacyButton = std::make_unique<DeclarativeUI::Components::Button>();
            auto commandButton = DeclarativeUI::Command::Adapters::ComponentSystemAdapter::instance().convertToCommand(legacyButton.get());
        }
        qint64 conversionTime = timer.elapsed();
        
        qDebug() << "Legacy component creation:" << legacyTime << "ms for" << iterations << "iterations";
        qDebug() << "Command creation:" << commandTime << "ms for" << iterations << "iterations";
        qDebug() << "Conversion overhead:" << conversionTime << "ms for" << iterations << "iterations";
        qDebug() << "Command overhead:" << ((double)commandTime / legacyTime - 1.0) * 100 << "%";
        qDebug() << "Conversion overhead:" << ((double)conversionTime / legacyTime - 1.0) * 100 << "%";
    }
};
