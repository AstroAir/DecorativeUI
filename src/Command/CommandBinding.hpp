...
public:
    ...
    void addStateBinding(BaseUICommand* command,
    ...
    void syncWidgetToCommand(BaseUICommand* command,
                             const QString& property = "");
    void syncCommandToState(BaseUICommand* command,
                            const QString& property = "");
    void syncStateToCommand(BaseUICommand* command,
                            const QString& property = "");
    ...
    BindingUpdateMode getDefaultUpdateMode() const {
...
signals:
    ...
    void propertyBindingRemoved(BaseUICommand* command,
                                const QString& property);
...
private:
    ...
    void connectPropertyBinding(BindingInfo* info,
                                const PropertyBindingConfig& config);
    void connectEventBinding(BindingInfo* info,
                             const EventBindingConfig& config);
    void connectStateBinding(BindingInfo* info,
                             const StateBindingConfig& config);

    void updateProperty(BaseUICommand* command, QWidget* widget,
                        const PropertyBindingConfig& config,
                        const QVariant& value);
    void scheduleUpdate(BaseUICommand* command, const QString& property,
                        const QVariant& value);
    void processUpdate(BaseUICommand* command, const QString& property,
                       const QVariant& value);

    QVariant convertValue(
        const QVariant& value,
...
inline void bindProperty(
    BaseUICommand* command, QWidget* widget, const QString& property,
    BindingDirection direction = BindingDirection::TwoWay) {
    PropertyBindingConfig config(property, property, direction);
    CommandBindingManager::instance().addPropertyBinding(command, widget,
                                                         config);
}

// **Property binding with different names**
inline void bindProperty(
    BaseUICommand* command, QWidget* widget, const QString& commandProperty,
    const QString& widgetProperty,
    BindingDirection direction = BindingDirection::TwoWay) {
...
// **State binding**
inline void bindToState(BaseUICommand* command, const QString& property,
