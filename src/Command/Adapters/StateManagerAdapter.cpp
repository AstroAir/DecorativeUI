/**
 * @file StateManagerAdapter.cpp
 * @brief Implementation of CommandStateManagerAdapter for Command-State
 * integration
 *
 * This file provides the implementation for bridging Command objects with the
 * StateManager system, enabling automatic synchronization between command
 * properties and application state. It supports bidirectional binding, batch
...
/**
 * @brief Construct a new CommandStateManagerAdapter
 * @param parent Parent QObject for memory management
 *
 * Initializes the adapter with the StateManager singleton and sets up
 * automatic synchronization mechanisms. Creates a timer for delayed
 * synchronization to batch multiple rapid changes.
 */
CommandStateManagerAdapter::CommandStateManagerAdapter(QObject* parent)
    ...
    // Initialize sync timer for batching rapid changes
    ...
    connect(sync_timer_, &QTimer::timeout, this,
            &CommandStateManagerAdapter::onDelayedSync);

    // Connect to state manager changes for automatic command updates
    connect(&state_manager_, &Binding::StateManager::stateChanged, this,
            &CommandStateManagerAdapter::onStateChanged);

    qDebug() << "🌐 CommandStateManagerAdapter initialized";
}

/**
 * @brief Get the singleton instance of CommandStateManagerAdapter
 * @return Reference to the singleton instance
 *
 * Provides thread-safe access to the singleton adapter instance.
 * The instance is created on first access and persists for the
...
/**
 * @brief Bind a command property to a state key
 * @param command Shared pointer to the command to bind
 * @param stateKey State key to bind to
 * @param commandProperty Property name on the command
 *
 * Creates a bidirectional binding between a command property and a state key.
 * If a binding already exists for the same command and property, the state key
 * is updated. The binding enables automatic synchronization between command
 * properties and application state.
 */
void CommandStateManagerAdapter::bindCommand(
    std::shared_ptr<UI::BaseUICommand> command, const QString& stateKey,
    ...
    // Create new binding structure
    CommandBinding binding;
    binding.command = command;
    binding.command_property = commandProperty;
    binding.state_key = stateKey;

    // Establish the actual binding connections
    establishBinding(binding);

    // Add to binding storage and update indices
    size_t bindingIndex = bindings_.size();
    bindings_.push_back(binding);

    // Update lookup indices for efficient access
    command_to_bindings_[command].push_back(bindingIndex);
    state_to_bindings_[stateKey].push_back(bindingIndex);

    // Notify observers and log the binding
    emit commandBound(command, stateKey);
    qDebug() << "🔗 Bound command property" << commandProperty << "to state"
             << stateKey;
}

void CommandStateManagerAdapter::unbindCommand(
    std::shared_ptr<UI::BaseUICommand> command, const QString& stateKey) {
    if (!command) {
        return;
    }
...
void CommandStateManagerAdapter::unbindAllCommands(
    std::shared_ptr<UI::BaseUICommand> command) {
    unbindCommand(command, "");
}

void CommandStateManagerAdapter::bindCommandProperties(
    std::shared_ptr<UI::BaseUICommand> command,
...
void CommandStateManagerAdapter::bindCommandHierarchy(
    std::shared_ptr<UI::BaseUICommand> rootCommand,
    const QString& statePrefix) {
    if (!rootCommand) {
        return;
    }

    // Bind root command
    QString rootStateKey =
        statePrefix.isEmpty()
            ? generateStateKey(rootCommand, "value")
            : QString("%1.%2").arg(statePrefix,
                                   generateStateKey(rootCommand, "value"));
    ...
    for (auto child : rootCommand->getChildren()) {
        QString childPrefix =
            QString("%1.%2").arg(rootStateKey, child->getId().toString());
        bindCommandHierarchy(child, childPrefix);
    }
}

void CommandStateManagerAdapter::syncCommandToState(
    std::shared_ptr<UI::BaseUICommand> command, const QString& property) {
    auto cmdIt = command_to_bindings_.find(command);
    if (cmdIt == command_to_bindings_.end()) {
        return;
    }
    ...
    for (size_t index : cmdIt->second) {
        if (index < bindings_.size()) {
            ...
            if (property.isEmpty() || binding.command_property == property) {
                try {
                    QVariant value = command->getState()->getProperty<QVariant>(
                        binding.command_property);
                    ...
                    handleSyncError(command,
                                    QString("Command to state sync failed: %1")
                                        .arg(e.what()));
                }
            }
        }
...
void CommandStateManagerAdapter::syncStateToCommand(
    std::shared_ptr<UI::BaseUICommand> command, const QString& property) {
    auto cmdIt = command_to_bindings_.find(command);
    if (cmdIt == command_to_bindings_.end()) {
        return;
    ...
    for (size_t index : cmdIt->second) {
        if (index < bindings_.size()) {
            ...
            if (property.isEmpty() || binding.command_property == property) {
                try {
                    auto statePtr =
                        state_manager_.getState<QVariant>(binding.state_key);
                    QVariant value = statePtr ? statePtr->get() : QVariant{};
                    command->getState()->setProperty(binding.command_property,
                                                     value);
                } catch (const std::exception& e) {
                    handleSyncError(command,
                                    QString("State to command sync failed: %1")
                                        .arg(e.what()));
                }
            }
        }
...
void CommandStateManagerAdapter::setCommandValidator(
    std::shared_ptr<UI::BaseUICommand> command, const QString& property,
...
void CommandStateManagerAdapter::removeCommandValidator(
    std::shared_ptr<UI::BaseUICommand> command, const QString& property) {
    size_t bindingIndex = findBinding(command, property);
    if (bindingIndex != SIZE_MAX) {
...
bool CommandStateManagerAdapter::validateCommandState(
    std::shared_ptr<UI::BaseUICommand> command, const QString& property) {
    auto cmdIt = command_to_bindings_.find(command);
    if (cmdIt == command_to_bindings_.end()) {
        return true;
            ...
            if (property.isEmpty() || binding.command_property == property) {
                if (binding.validator) {
                    ...
                    if (!binding.validator(value)) {
                        emit validationFailed(command, binding.command_property,
                                              "Validation failed");
                        return false;
                    }
                }
...
QStringList CommandStateManagerAdapter::getBoundStateKeys(
    std::shared_ptr<UI::BaseUICommand> command) const {
    QStringList keys;
    auto cmdIt = command_to_bindings_.find(command);
    if (cmdIt != command_to_bindings_.end()) {
        for (size_t index : cmdIt->second) {
...
CommandStateManagerAdapter::getCommandsForState(const QString& stateKey) const {
...
void CommandStateManagerAdapter::establishBinding(CommandBinding& binding) {
    ...
    binding.command_connection =
        connect(binding.command.get(), &UI::BaseUICommand::propertyChanged,
...
void CommandStateManagerAdapter::removeBinding(size_t bindingIndex) {
    ...
    if (cmdIt != command_to_bindings_.end()) {
        ...
        indices.erase(std::remove(indices.begin(), indices.end(), bindingIndex),
                      indices.end());
        if (indices.empty()) {
            command_to_bindings_.erase(cmdIt);
        }
...
void CommandStateManagerAdapter::scheduleSync(
    std::shared_ptr<UI::BaseUICommand> command) {
    if (!auto_sync_enabled_) {
        return;
    }
...
void CommandStateManagerAdapter::performSync(
    std::shared_ptr<UI::BaseUICommand> command) {
    syncCommandToState(command, "");
}

QString CommandStateManagerAdapter::generateStateKey(
    std::shared_ptr<UI::BaseUICommand> command, const QString& property) {
    return QString("%1_%2_%3")
        .arg(command->getCommandType(), command->getId().toString(), property);
}

void CommandStateManagerAdapter::validateBinding(
...
void CommandStateManagerAdapter::handleSyncError(
    std::shared_ptr<UI::BaseUICommand> command, const QString& error) {
    qWarning() << "State sync error:" << error;
    emit syncError(command, error);
}

size_t CommandStateManagerAdapter::findBinding(
    std::shared_ptr<UI::BaseUICommand> command, const QString& property) {
    auto cmdIt = command_to_bindings_.find(command);
    if (cmdIt == command_to_bindings_.end()) {
        return SIZE_MAX;
    ...
    for (size_t index : cmdIt->second) {
        if (index < bindings_.size() &&
            bindings_[index].command_property == property) {
            return index;
        }
    }
...
void CommandStateManagerAdapter::onStateChanged(const QString& key,
    ...
    if (stateIt != state_to_bindings_.end()) {
        for (size_t index : stateIt->second) {
            if (index < bindings_.size()) {
                const auto& binding = bindings_[index];
                if (binding.command) {
                    binding.command->getState()->setProperty(
                        binding.command_property, value);
                    emit commandStateChanged(binding.command,
                                             binding.command_property, value);
                }
            }
        }
...
void CommandStateManagerAdapter::onCommandPropertyChanged(
