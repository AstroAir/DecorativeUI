...
/**
 * @brief Adapter that wraps existing UIElement components to work with the
 * Command system
 *
...
public:
    explicit UIElementCommandAdapter(std::unique_ptr<Core::UIElement> element,
...
/**
 * @brief Factory for creating Command adapters from existing UIElement
 * components
 */
class UIElementAdapterFactory {
public:
    // **Create adapter from UIElement**
    static std::shared_ptr<UIElementCommandAdapter> createAdapter(
    ...
    static std::shared_ptr<UIElementCommandAdapter> createAdapterByType(
        const QString& elementType);

private:
    static std::unordered_map<QString,
                              std::function<std::unique_ptr<Core::UIElement>()>>
        element_factories_;
};
...
public:
    explicit CommandUIElementAdapter(std::shared_ptr<UI::BaseUICommand> command,
                                     QObject* parent = nullptr);
...
/**
 * @brief Migration helper for converting existing UIElement code to Command
 * system
 */
class UIElementMigrationHelper {
public:
    // **Convert existing UIElement hierarchy to Command hierarchy**
    static std::shared_ptr<UI::BaseUICommand> convertToCommandHierarchy(
        Core::UIElement* rootElement);

    // **Convert Command hierarchy to UIElement hierarchy**
    static std::unique_ptr<Core::UIElement> convertToUIElementHierarchy(
        std::shared_ptr<UI::BaseUICommand> rootCommand);

    // **Migrate property bindings**
    static void migratePropertyBindings(
        Core::UIElement* element, std::shared_ptr<UI::BaseUICommand> command);

    // **Migrate event handlers**
    static void migrateEventHandlers(
        Core::UIElement* element, std::shared_ptr<UI::BaseUICommand> command);
    ...
    // **Validate migration**
    static bool validateMigration(Core::UIElement* original,
                                  std::shared_ptr<UI::BaseUICommand> migrated);
};
...
public:
    ...
    static std::shared_ptr<UI::BaseUICommand> createCommandFromComponent(
        const QString& componentType);

    // **Create Component from Command**
    static std::unique_ptr<Core::UIElement> createComponentFromCommand(
        std::shared_ptr<UI::BaseUICommand> command);

    // **Bidirectional synchronization**
    static void establishBidirectionalSync(
...
template <typename T>
CommandUIElementAdapter& CommandUIElementAdapter::setCommandProperty(
...
template <typename T>
T CommandUIElementAdapter::getCommandProperty(const QString& name,
...
// **Quick conversion**
inline std::shared_ptr<UI::BaseUICommand> toCommand(
    std::unique_ptr<Core::UIElement> element) {
    return UIElementAdapterFactory::createAdapter(std::move(element));
}

inline std::unique_ptr<Core::UIElement> toUIElement(
    std::shared_ptr<UI::BaseUICommand> command) {
    return std::make_unique<CommandUIElementAdapter>(command);
}

// **Bidirectional bridge**
inline void bridge(Core::UIElement* element,
                   std::shared_ptr<UI::BaseUICommand> command) {
