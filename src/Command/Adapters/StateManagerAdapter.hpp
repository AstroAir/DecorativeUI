...
/**
 * @brief Enhanced StateManager adapter that provides seamless integration with
 * the Command system
 *
 * This adapter extends the existing StateManager to work optimally with Command
 * objects, providing automatic binding, batch updates, and command-specific
...
public:
    ...
    void bindCommand(std::shared_ptr<UI::BaseUICommand> command,
                     const QString& stateKey,
                     const QString& commandProperty = "value");
    void unbindCommand(std::shared_ptr<UI::BaseUICommand> command,
                       const QString& stateKey = "");
    void unbindCommand(std::shared_ptr<UI::BaseUICommand> command,
    ...
    void bindCommandProperties(
        std::shared_ptr<UI::BaseUICommand> command,
        const std::unordered_map<QString, QString>& propertyToStateMap);
    void bindCommandHierarchy(std::shared_ptr<UI::BaseUICommand> rootCommand,
                              const QString& statePrefix = "");

    // **State synchronization**
    void syncCommandToState(std::shared_ptr<UI::BaseUICommand> command,
                            const QString& property = "");
    void syncStateToCommand(std::shared_ptr<UI::BaseUICommand> command,
                            const QString& property = "");
    ...
    template <typename T>
    void setCommandState(std::shared_ptr<UI::BaseUICommand> command,
                         const QString& property, const T& value);

    template <typename T>
    T getCommandState(std::shared_ptr<UI::BaseUICommand> command,
                      const QString& property, const T& defaultValue = T{});
    ...
    void setCommandValidator(std::shared_ptr<UI::BaseUICommand> command,
                             const QString& property,
                             std::function<bool(const QVariant&)> validator);
    void removeCommandValidator(std::shared_ptr<UI::BaseUICommand> command,
                                const QString& property);
    bool validateCommandState(std::shared_ptr<UI::BaseUICommand> command,
                              const QString& property = "");

    // **State persistence for commands**
    void saveCommandState(std::shared_ptr<UI::BaseUICommand> command,
                          const QString& filePath);
    void loadCommandState(std::shared_ptr<UI::BaseUICommand> command,
                          const QString& filePath);
    ...
    QStringList getBoundStateKeys(
        std::shared_ptr<UI::BaseUICommand> command) const;
    std::vector<std::shared_ptr<UI::BaseUICommand>> getCommandsForState(
    ...
    const Binding::StateManager& getStateManager() const {
...
signals:
    void commandBound(std::shared_ptr<UI::BaseUICommand> command,
                      const QString& stateKey);
    void commandUnbound(std::shared_ptr<UI::BaseUICommand> command,
                        const QString& stateKey);
    void commandStateChanged(std::shared_ptr<UI::BaseUICommand> command,
                             const QString& property, const QVariant& value);
    void batchUpdateStarted();
    void batchUpdateCommitted();
    void batchUpdateRolledBack();
    void validationFailed(std::shared_ptr<UI::BaseUICommand> command,
                          const QString& property, const QString& error);
    void syncError(std::shared_ptr<UI::BaseUICommand> command,
                   const QString& error);
...
private:
    ...
    std::unordered_map<std::shared_ptr<UI::BaseUICommand>, std::vector<size_t>>
        command_to_bindings_;
    ...
    QTimer* sync_timer_;
    std::unordered_set<std::shared_ptr<UI::BaseUICommand>>
        pending_sync_commands_;
    ...
    QString generateStateKey(std::shared_ptr<UI::BaseUICommand> command,
                             const QString& property);
    void validateBinding(const CommandBinding& binding);
    void handleSyncError(std::shared_ptr<UI::BaseUICommand> command,
                         const QString& error);

    size_t findBinding(std::shared_ptr<UI::BaseUICommand> command,
                       const QString& property);
    void updateBindingIndices();
};

/**
 * @brief Property binding adapter that integrates existing PropertyBinding with
 * Commands
 */
...
public:
    ...
    static std::unique_ptr<Binding::IPropertyBinding> createBinding(
        std::shared_ptr<UI::BaseUICommand> command,
        const QString& commandProperty, QObject* target,
        const QString& targetProperty);

    // **Create bidirectional binding**
    static std::pair<std::unique_ptr<Binding::IPropertyBinding>,
                     std::unique_ptr<Binding::IPropertyBinding>>
    createBidirectionalBinding(std::shared_ptr<UI::BaseUICommand> command,
                               const QString& commandProperty, QObject* target,
                               const QString& targetProperty);

    // **Batch binding creation**
    static std::vector<std::unique_ptr<Binding::IPropertyBinding>>
    createBindings(
        std::shared_ptr<UI::BaseUICommand> command,
        const std::vector<std::pair<QString, std::pair<QObject*, QString>>>&
            bindings);

    // **Integration with existing binding system**
    static void integrateWithExistingBindings(
        std::shared_ptr<UI::BaseUICommand> command);

private:
    static void setupCommandToTargetBinding(
        std::shared_ptr<UI::BaseUICommand> command,
        const QString& commandProperty, QObject* target,
        const QString& targetProperty);
    static void setupTargetToCommandBinding(
...
public:
    ...
    static void registerCommandValidator(
        const QString& commandType, const QString& property,
        std::function<bool(const QVariant&)> validator);
    ...
    static std::function<bool(const QVariant&)> rangeValidator(
        const QVariant& min, const QVariant& max);
    static std::function<bool(const QVariant&)> regexValidator(
        const QString& pattern);
    static std::function<bool(const QVariant&)> lengthValidator(
        int minLength, int maxLength = -1);
...
template <typename T>
void CommandStateManagerAdapter::setCommandState(
...
template <typename T>
T CommandStateManagerAdapter::getCommandState(
    std::shared_ptr<UI::BaseUICommand> command, const QString& property,
...
inline void bind(std::shared_ptr<UI::BaseUICommand> command,
                 const QString& stateKey, const QString& property = "value") {
    CommandStateManagerAdapter::instance().bindCommand(command, stateKey,
                                                       property);
}

inline void unbind(std::shared_ptr<UI::BaseUICommand> command,
                   const QString& stateKey = "") {
...
// **Quick state access**
template <typename T>
inline void set(std::shared_ptr<UI::BaseUICommand> command,
                const QString& property, const T& value) {
    CommandStateManagerAdapter::instance().setCommandState(command, property,
                                                           value);
}

template <typename T>
inline T get(std::shared_ptr<UI::BaseUICommand> command,
             const QString& property, const T& defaultValue = T{}) {
    return CommandStateManagerAdapter::instance().getCommandState(
...
// **Template implementations**
template <typename T>
void CommandStateManagerAdapter::createState(const QString& stateKey,
                                             const T& initialValue) {
    state_manager_.setState(stateKey, QVariant::fromValue(initialValue));
    qDebug() << "🆕 Created state:" << stateKey
