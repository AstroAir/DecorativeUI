...
public:
    ...
    QPoint getPosition() const { return getData("position", QPoint{}).toPoint(); }
...
public:
    ...
    void setCursorPosition(int position) { setData("cursorPosition", position); }
    int getCursorPosition() const { return getData("cursorPosition", -1).toInt(); }
...
public:
    explicit ValidationEvent(CommandEventType type, BaseUICommand* source = nullptr);
    ...
    void setErrorMessage(const QString& message) { setData("errorMessage", message); }
    QString getErrorMessage() const { return getData("errorMessage").toString(); }
...
public:
    explicit CustomEvent(const QString& customType, BaseUICommand* source = nullptr);
...
struct EventHandlerRegistration {
    ...
    EventHandlerRegistration(CommandEventType type, CommandEventHandler h, 
...
public:
    ...
    QUuid registerHandler(BaseUICommand* command, CommandEventType eventType, 
                         CommandEventHandler handler, CommandEventPriority priority = CommandEventPriority::Normal);
    QUuid registerHandler(BaseUICommand* command, const EventHandlerRegistration& registration);
    
    // **Filtered event handlers**
    QUuid registerFilteredHandler(BaseUICommand* command, CommandEventType eventType, 
                                 CommandEventHandler handler, CommandEventFilter filter,
                                 CommandEventPriority priority = CommandEventPriority::Normal);
    
    // **One-time event handlers**
    QUuid registerOnceHandler(BaseUICommand* command, CommandEventType eventType, 
                             CommandEventHandler handler, CommandEventPriority priority = CommandEventPriority::Normal);
    
    // **Handler management**
    void unregisterHandler(const QUuid& handlerId);
    void unregisterAllHandlers(BaseUICommand* command);
    void unregisterHandlersByType(BaseUICommand* command, CommandEventType eventType);
    
    // **Global event filters**
    void addGlobalFilter(CommandEventFilter filter, CommandEventPriority priority = CommandEventPriority::Normal);
    void removeGlobalFilter(CommandEventFilter filter);
    
    // **Event interception**
    void addEventInterceptor(CommandEventType eventType, CommandEventHandler interceptor);
...
signals:
    ...
    void handlerUnregistered(BaseUICommand* command, CommandEventType eventType);
...
private:
    ...
    std::vector<std::pair<CommandEventFilter, CommandEventPriority>> global_filters_;
...
inline std::unique_ptr<ValueChangeEvent> createValueChangeEvent(BaseUICommand* source, 
...
inline std::unique_ptr<TextChangeEvent> createTextChangeEvent(BaseUICommand* source, 
...
inline std::unique_ptr<CustomEvent> createCustomEvent(BaseUICommand* source, const QString& customType) {
...
inline void dispatchValueChange(BaseUICommand* source, const QVariant& oldValue, const QVariant& newValue) {
...
inline QUuid onClick(BaseUICommand* command, std::function<void()> handler) {
    return CommandEventDispatcher::instance().registerHandler(command, CommandEventType::Clicked,
        [handler](const CommandEvent&) { if (handler) handler(); });
}

inline QUuid onValueChanged(BaseUICommand* command, std::function<void(const QVariant&)> handler) {
    return CommandEventDispatcher::instance().registerHandler(command, CommandEventType::ValueChanged,
...
inline QUuid onTextChanged(BaseUICommand* command, std::function<void(const QString&)> handler) {
    return CommandEventDispatcher::instance().registerHandler(command, CommandEventType::TextChanged,
        [handler](const CommandEvent& event) {
            if (handler) {
