...
/**
 * @file SearchBox.hpp
 * @brief Advanced search input component with auto-completion, filtering, and
 * search history
 *
...
struct SearchSuggestion {
    ...
    SearchSuggestion(const QString& text, const QString& desc = QString(),
                     ...
                     const QVariant& data = QVariant(), int priority = 0)
        : text(text),
          description(desc),
          icon(icon),
          data(data),
...
public:
    ...
    SearchBox& onSuggestionSelected(
        std::function<void(const SearchSuggestion&)> handler);
    SearchBox& onSuggestionRequested(
...
private:
    ...
    std::function<QList<SearchSuggestion>(const QString&)>
        suggestion_requested_handler_;
...
public:
    explicit SuggestionItemWidget(const SearchSuggestion& suggestion,
