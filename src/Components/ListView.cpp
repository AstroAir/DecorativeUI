...
ListView::ListView(QObject* parent)
    : UIElement(parent),
      list_view_widget_(nullptr),
      string_list_model_(nullptr) {}
...
ListView& ListView::selectionBehavior(
...
ListView& ListView::editTriggers(QAbstractItemView::EditTriggers triggers) {
    return static_cast<ListView&>(
        setProperty("editTriggers", static_cast<int>(triggers)));
}

ListView& ListView::dragDropMode(QAbstractItemView::DragDropMode mode) {
    return static_cast<ListView&>(
        setProperty("dragDropMode", static_cast<int>(mode)));
}

ListView& ListView::defaultDropAction(Qt::DropAction action) {
    return static_cast<ListView&>(
        setProperty("defaultDropAction", static_cast<int>(action)));
}
...
ListView& ListView::movement(QListView::Movement movement) {
    return static_cast<ListView&>(
        setProperty("movement", static_cast<int>(movement)));
}
...
ListView& ListView::resizeMode(QListView::ResizeMode mode) {
    return static_cast<ListView&>(
        setProperty("resizeMode", static_cast<int>(mode)));
}

ListView& ListView::layoutMode(QListView::LayoutMode mode) {
    return static_cast<ListView&>(
        setProperty("layoutMode", static_cast<int>(mode)));
}
...
ListView& ListView::onDoubleClicked(
...
ListView& ListView::onIconSizeChanged(
    std::function<void(const QSize&)> handler) {
...
void ListView::initialize() {
    if (!list_view_widget_) {
        ...
        if (clicked_handler_) {
            connect(
                list_view_widget_, &QListView::clicked, this,
        ...
        if (double_clicked_handler_) {
            connect(list_view_widget_, &QListView::doubleClicked, this,
                    [this](const QModelIndex& index) {
                        double_clicked_handler_(index);
                    });
        }

        if (pressed_handler_) {
            connect(
                list_view_widget_, &QListView::pressed, this,
        ...
        if (activated_handler_) {
            connect(list_view_widget_, &QListView::activated, this,
                    [this](const QModelIndex& index) {
                        activated_handler_(index);
                    });
        }

        if (entered_handler_) {
            connect(
                list_view_widget_, &QListView::entered, this,
        ...
        if (icon_size_changed_handler_) {
            connect(list_view_widget_, &QListView::iconSizeChanged, this,
                    [this](const QSize& size) {
                        icon_size_changed_handler_(size);
                    });
        }
    }
}
...
QModelIndex ListView::getCurrentIndex() const {
    return list_view_widget_ ? list_view_widget_->currentIndex()
                             : QModelIndex();
}

QModelIndexList ListView::getSelectedIndexes() const {
    return list_view_widget_
               ? list_view_widget_->selectionModel()->selectedIndexes()
               : QModelIndexList();
}
...
void ListView::scrollTo(const QModelIndex& index,
...
void ListView::removeItem(int row) {
    if (string_list_model_ && row >= 0 &&
        row < string_list_model_->rowCount()) {
