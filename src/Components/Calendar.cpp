...
Calendar::Calendar(QObject* parent)
    : UIElement(parent),
      calendar_widget_(nullptr),
      multi_selection_enabled_(false),
      highlight_today_enabled_(true) {}
...
Calendar& Calendar::firstDayOfWeek(Qt::DayOfWeek dayOfWeek) {
    return static_cast<Calendar&>(
        setProperty("firstDayOfWeek", static_cast<int>(dayOfWeek)));
}
...
Calendar& Calendar::verticalHeaderFormat(
    QCalendarWidget::VerticalHeaderFormat format) {
    return static_cast<Calendar&>(
        setProperty("verticalHeaderFormat", static_cast<int>(format)));
}

Calendar& Calendar::horizontalHeaderFormat(
    QCalendarWidget::HorizontalHeaderFormat format) {
    return static_cast<Calendar&>(
        setProperty("horizontalHeaderFormat", static_cast<int>(format)));
}
...
Calendar& Calendar::onCurrentPageChanged(
...
Calendar& Calendar::weekdayTextFormat(Qt::DayOfWeek dayOfWeek,
...
Calendar& Calendar::dateTextFormat(const QDate& date,
                                   const QTextCharFormat& format) {
    if (calendar_widget_) {
...
Calendar& Calendar::highlightToday(bool highlight) {
    ...
    if (calendar_widget_ && highlight) {
        ...
        todayFormat.setBackground(
            QColor(255, 255, 0, 100));  // Light yellow highlight
...
Calendar& Calendar::showWeekNumbers(bool show) {
    if (calendar_widget_) {
        calendar_widget_->setVerticalHeaderFormat(
            show ? QCalendarWidget::ISOWeekNumbers
...
Calendar& Calendar::removeSpecialDate(const QDate& date) {
    ...
    if (calendar_widget_) {
        calendar_widget_->setDateTextFormat(date,
                                            QTextCharFormat());  // Reset format
    }
    return *this;
}
...
Calendar& Calendar::setHolidays(const QList<QDate>& holidays) {
    ...
    if (calendar_widget_) {
        ...
        holidayFormat.setBackground(
            QColor(255, 200, 200, 100));  // Light red background
...
Calendar& Calendar::onDateDoubleClicked(
...
void Calendar::initialize() {
    if (!calendar_widget_) {
        ...
        if (current_page_changed_handler_) {
            connect(calendar_widget_, &QCalendarWidget::currentPageChanged,
                    this, [this](int year, int month) {
                        current_page_changed_handler_(year, month);
                    });
        }
    }
}
...
QDate Calendar::getSelectedDate() const {
    return calendar_widget_ ? calendar_widget_->selectedDate()
                            : QDate::currentDate();
}
...
QList<QDate> Calendar::getSelectedDates() const { return selected_dates_; }
...
QTextCharFormat Calendar::getWeekdayTextFormat(Qt::DayOfWeek dayOfWeek) const {
    return calendar_widget_ ? calendar_widget_->weekdayTextFormat(dayOfWeek)
                            : QTextCharFormat();
}

QTextCharFormat Calendar::getDateTextFormat(const QDate& date) const {
    return calendar_widget_ ? calendar_widget_->dateTextFormat(date)
                            : QTextCharFormat();
}

bool Calendar::isWeekNumbersShown() const {
    return calendar_widget_ ? (calendar_widget_->verticalHeaderFormat() ==
...
void Calendar::clearSelection() {
    ...
    if (calendar_widget_) {
        ...
        for (const QDate& date : special_dates_) {
            QTextCharFormat specialFormat;
            specialFormat.setBackground(
                QColor(255, 165, 0, 100));  // Restore special dates
            calendar_widget_->setDateTextFormat(date, specialFormat);
        }
...
void Calendar::selectDateRange(const QDate& start, const QDate& end) {
    if (!multi_selection_enabled_ || !start.isValid() || !end.isValid())
        return;
