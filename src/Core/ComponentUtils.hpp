...
public:
    ...
    bool setProperty(const char* name, ValueType&& value) {
        if (!target_) {
            UI_LOG_ERROR(QString("Attempted to set property '%1' on null object").arg(name));
            return false;
        }

        UI_ERROR_SCOPE("PropertySetter", QString("Setting property '%1'").arg(name));
        ...
        try {
            ...
            UI_LOG_ERROR(QString("Exception while setting property '%1': %2")
                        .arg(name).arg(QString::fromStdString(e.what())));
            return false;
        }
    }
...
[[nodiscard]] std::unique_ptr<LayoutType> createLayout(Args&&... args) {
    return UI_SAFE_EXECUTE([&]() {
        return std::make_unique<LayoutType>(std::forward<Args>(args)...);
    }, "Creating layout").value_or(nullptr);
}
...
bool applyLayout(WidgetType* widget, std::unique_ptr<LayoutType> layout) {
    ...
    try {
        ...
        UI_LOG_ERROR(QString("Failed to apply layout: %1").arg(QString::fromStdString(e.what())));
        return false;
    }
}
...
bool addWidget(LayoutType* layout, WidgetType* widget, int position = -1) {
    ...
    try {
        ...
        UI_LOG_ERROR(QString("Failed to add widget to layout: %1").arg(QString::fromStdString(e.what())));
        return false;
    }
}
...
bool addWidgetToGrid(QGridLayout* layout, WidgetType* widget, int row, int column,
    ...
    if (row < 0 || column < 0) {
        UI_LOG_ERROR(QString("Invalid grid position: row=%1, column=%2").arg(row).arg(column));
        return false;
    }

    UI_ERROR_SCOPE("LayoutUtils", QString("Adding widget to grid at (%1,%2)").arg(row).arg(column));

    try {
        layout->addWidget(widget, row, column, rowSpan, columnSpan, alignment);
        return true;
    } catch (const std::exception& e) {
        UI_LOG_ERROR(QString("Failed to add widget to grid: %1").arg(QString::fromStdString(e.what())));
        return false;
    }
}
...
bool addFormRow(QFormLayout* layout, LabelType* label, FieldType* field) {
    ...
    try {
        ...
        UI_LOG_ERROR(QString("Failed to add form row: %1").arg(QString::fromStdString(e.what())));
        return false;
    }
}

bool addFormRow(QFormLayout* layout, const QString& label, QWidget* field) {
    if (!layout || !field) {
        UI_LOG_ERROR("Cannot add null field to form layout");
        return false;
    }

    UI_ERROR_SCOPE("LayoutUtils", QString("Adding labeled row '%1' to form layout").arg(label));

    try {
        layout->addRow(label, field);
        return true;
    } catch (const std::exception& e) {
        UI_LOG_ERROR(QString("Failed to add labeled form row: %1").arg(QString::fromStdString(e.what())));
        return false;
    }
}
...
void applyThemeStyle(WidgetType* widget, const QString& component_name = {}) {
    ...
    UI_ERROR_SCOPE("StylingUtils", QString("Applying theme to %1").arg(component_name));
...
void addDropShadow(WidgetType* widget, const QColor& color = QColor(0, 0, 0, 80),
    ...
    try {
        ...
        UI_LOG_ERROR(QString("Failed to add drop shadow: %1").arg(QString::fromStdString(e.what())));
    }
}
...
void setBorderRadius(WidgetType* widget, int radius) {
    ...
    UI_ERROR_SCOPE("StylingUtils", QString("Setting border radius to %1px").arg(radius));
    ...
    try {
        ...
        UI_LOG_ERROR(QString("Failed to set border radius: %1").arg(QString::fromStdString(e.what())));
    }
}
...
bool connectSafely(Sender* sender, Signal signal, Receiver* receiver, Slot slot,
    ...
    try {
        ...
        UI_LOG_ERROR(QString("Failed to connect signal: %1").arg(QString::fromStdString(e.what())));
        return false;
    }
}
...
void executeDelayed(int delay_ms, F&& func) {
    UI_ERROR_SCOPE("EventUtils", QString("Scheduling delayed execution (%1ms)").arg(delay_ms));
    ...
    try {
        ...
        UI_LOG_ERROR(QString("Failed to schedule delayed execution: %1").arg(QString::fromStdString(e.what())));
    }
}
...
template<Concepts::QtWidget WidgetType>
[[nodiscard]] bool validateWidget(WidgetType* widget, const QString& widget_name = {}) {
    if (!widget) {
    ...
    if (!widget->isValid()) {
        UI_LOG_ERROR(QString("Widget '%1' is in invalid state").arg(widget_name));
        return false;
    }
...
[[nodiscard]] bool validateLayoutHierarchy(QWidget* root_widget) {
    if (!root_widget) {
    ...
    try {
        ...
        std::function<bool(QWidget*)> validateRecursive = [&](QWidget* widget) -> bool {
            ...
            for (QObject* child : widget->children()) {
                if (auto* childWidget = qobject_cast<QWidget*>(child)) {
                    if (childWidget->parent() != widget) {
                        UI_LOG_WARNING(QString("Widget parent-child relationship inconsistency detected"));
                        return false;
                    }
        ...
        UI_LOG_ERROR(QString("Exception during layout validation: %1").arg(QString::fromStdString(e.what())));
        return false;
    }
}
