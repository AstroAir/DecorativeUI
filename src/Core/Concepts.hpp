...
template<typename T>
concept PropertyValue = std::convertible_to<T, QVariant> || 
...
template<typename T>
concept StringLike = std::convertible_to<T, QString> || 
                    std::convertible_to<T, const char*> ||
                    std::same_as<T, std::string>;

// **Callable Concepts**
template<typename F>
concept VoidCallback = std::invocable<F> && 
...
// **Animation Concepts**
template<typename T>
concept AnimatableProperty = PropertyValue<T> && requires(T t) {
...
// **Styling Concepts**
template<typename T>
concept ColorLike = std::convertible_to<T, QColor> ||
                   StringLike<T>;

template<typename T>
concept FontLike = std::convertible_to<T, QFont> ||
                  StringLike<T>;

template<typename T>
concept SizeLike = std::convertible_to<T, QSize> ||
                  requires(T t) {
...
// **Geometry Concepts**
template<typename T>
concept PointLike = std::convertible_to<T, QPoint> ||
                   requires(T t) {
...
template<typename T>
concept RectLike = std::convertible_to<T, QRect> ||
                  requires(T t) {
                      { t.x() } -> std::convertible_to<int>;
                      { t.y() } -> std::convertible_to<int>;
                      { t.width() } -> std::convertible_to<int>;
...
template<typename T>
concept ConfigurationStruct = std::is_aggregate_v<T> && 
...
template<typename T>
concept UniquePointer = SmartPointer<T> && 
                       std::same_as<T, std::unique_ptr<typename T::element_type>>;

template<typename T>
concept SharedPointer = SmartPointer<T> && 
                       std::same_as<T, std::shared_ptr<typename T::element_type>>;

// **RAII Concepts**
template<typename T>
concept RAIIResource = std::is_destructible_v<T> && 
