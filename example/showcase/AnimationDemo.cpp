...
void AnimationDemo::createAnimationControls() {
    ...
    animation_type_->addItems({"Fade In/Out", "Slide Left/Right", "Scale Up/Down", "Rotate"});
    ...
    // Connect signals
    connect(start_button_, &QPushButton::clicked, this, &AnimationDemo::onStartAnimation);
    connect(stop_button_, &QPushButton::clicked, this, &AnimationDemo::onStopAnimation);
    connect(animation_type_, &QComboBox::currentTextChanged, this, &AnimationDemo::onAnimationTypeChanged);
    connect(duration_slider_, &QSlider::valueChanged, [duration_label](int value) {
...
void AnimationDemo::createAnimationTarget() {
    ...
    auto description = new QLabel(
        ...
        "Select an animation type and click 'Start Animation' to see smooth transitions."
    );
...
void AnimationDemo::onStartAnimation() {
    ...
    qDebug() << "Starting animation:" << type << "Duration:" << duration << "ms";
    
    // Simulate animation with a timer (real implementation would use Animation::AnimationEngine)
...
void AnimationDemo::onAnimationTypeChanged() {
    ...
    if (type.contains("Fade")) color = "#9b59b6";
    else if (type.contains("Slide")) color = "#e67e22";
    else if (type.contains("Scale")) color = "#27ae60";
    else if (type.contains("Rotate")) color = "#f39c12";
    
    animation_target_->setStyleSheet(QString(
        "QWidget { "
        "background-color: %1; "
        "border: 2px solid %1; "
        "border-radius: 8px; "
