...
void ThemeManager::createThemeSelector() {
    ...
    // Connect signals
    connect(theme_combo_, &QComboBox::currentTextChanged, this, &ThemeManager::onThemeSelected);
    connect(preview_button_, &QPushButton::clicked, this, &ThemeManager::onPreviewTheme);
    connect(apply_button_, &QPushButton::clicked, this, &ThemeManager::onApplyTheme);
    connect(reset_button_, &QPushButton::clicked, this, &ThemeManager::onResetTheme);
...
void ThemeManager::createColorCustomizer() {
    ...
    primary_button->setStyleSheet("QPushButton { background-color: #3498db; color: white; }");
    secondary_button->setStyleSheet("QPushButton { background-color: #2c3e50; color: white; }");
    accent_button->setStyleSheet("QPushButton { background-color: #e74c3c; color: white; }");
    ...
    connect(primary_button, &QPushButton::clicked, this, &ThemeManager::onCustomColorChanged);
    connect(secondary_button, &QPushButton::clicked, this, &ThemeManager::onCustomColorChanged);
    connect(accent_button, &QPushButton::clicked, this, &ThemeManager::onCustomColorChanged);
...
void ThemeManager::createPreview() {
    ...
    auto description = new QLabel(
        "Select a theme from the dropdown and click 'Preview' to see how it looks. "
...
void ThemeManager::loadAvailableThemes() {
    theme_combo_->addItems({"Light", "Dark", "Blue", "Green", "Purple", "Custom"});
...
void ThemeManager::onCustomColorChanged() {
    ...
    if (color.isValid()) {
        button->setStyleSheet(QString("QPushButton { background-color: %1; color: white; }").arg(color.name()));
