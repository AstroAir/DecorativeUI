...
void ShowcaseApp::setupMenuBar() {
    ...
    connect(export_action, &QAction::triggered, this, &ShowcaseApp::onExportSettings);
    
    auto import_action = file_menu->addAction("&Import Settings...");
    import_action->setShortcut(QKeySequence::Open);
    connect(import_action, &QAction::triggered, this, &ShowcaseApp::onImportSettings);
    ...
    connect(light_action, &QAction::triggered, [this]() { onThemeChanged("light"); });
    connect(dark_action, &QAction::triggered, [this]() { onThemeChanged("dark"); });
    connect(auto_action, &QAction::triggered, [this]() { onThemeChanged("auto"); });
    
    view_menu->addSeparator();
    
    auto reset_action = view_menu->addAction("&Reset to Defaults");
    connect(reset_action, &QAction::triggered, this, &ShowcaseApp::onResetToDefaults);
    ...
    connect(performance_action, &QAction::toggled, this, &ShowcaseApp::onPerformanceToggled);
    ...
    connect(hot_reload_action, &QAction::toggled, this, &ShowcaseApp::onHotReloadToggled);
...
void ShowcaseApp::setupStatusBar() {
    ...
    connect(language_combo_, &QComboBox::currentTextChanged, this, &ShowcaseApp::onLanguageChanged);
    ...
    statusBar()->showMessage(QString("Initialized in %1ms").arg(startup_time_ms_), 3000);
}
...
void ShowcaseApp::setupNavigationBar() {
    try {
        ...
        navigation_bar_->addItem("Components", "🧩", "Explore all UI components");
...
void ShowcaseApp::setupDemoTabs() {
    ...
    try {
        ...
        connect(demo_tabs_, &QTabWidget::currentChanged, this, &ShowcaseApp::onTabChanged);
...
void ShowcaseApp::setupStateManagement() {
    ...
    state_manager_->setState("showcase.performance_monitoring", performance_monitoring_enabled_);
...
void ShowcaseApp::setupHotReload() {
    ...
    try {
        ...
        QStringList ui_files = {
            "resources/ui/main_window.json",
...
void ShowcaseApp::setupPerformanceMonitoring() {
    if (!performance_monitoring_enabled_) return;
    
    performance_timer_ = new QTimer(this);
    connect(performance_timer_, &QTimer::timeout, this, &ShowcaseApp::updatePerformanceStatus);
...
void ShowcaseApp::onThemeChanged(const QString& theme) {
    ...
    theme_button_->setText(QString("%1 Theme").arg(
        theme.left(1).toUpper() + theme.mid(1)
    ));
...
void ShowcaseApp::onLanguageChanged(const QString& language) {
    ...
    statusBar()->showMessage(QString("Language changed to %1").arg(language), 2000);
}
...
void ShowcaseApp::onExportSettings() {
    QString filename = QFileDialog::getSaveFileName(
        ...
        QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation) + "/showcase_settings.json",
        "JSON Files (*.json)"
    );
...
void ShowcaseApp::onAbout() {
    QMessageBox::about(this, "About DeclarativeUI Showcase",
        ...
        "<p>A comprehensive demonstration of the DeclarativeUI framework features.</p>"
        "<p><b>Features:</b></p>"
        "<ul>"
...
void ShowcaseApp::loadSettings() {
    ...
    performance_monitoring_enabled_ = settings.value("performance_monitoring", true).toBool();
...
void ShowcaseApp::saveSettings() {
    ...
    settings.setValue("performance_monitoring", performance_monitoring_enabled_);
...
void ShowcaseApp::updateWindowTitle() {
    setWindowTitle(QString("DeclarativeUI Showcase - %1 Theme").arg(
        current_theme_.left(1).toUpper() + current_theme_.mid(1)
    ));
}
