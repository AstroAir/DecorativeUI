...
void PerformanceMonitor::createControls() {
    ...
    connect(start_button_, &QPushButton::clicked, this,
            &PerformanceMonitor::onStartMonitoring);
    connect(stop_button_, &QPushButton::clicked, this,
            &PerformanceMonitor::onStopMonitoring);
    connect(clear_button_, &QPushButton::clicked, this,
            &PerformanceMonitor::onClearMetrics);

    main_layout_->addWidget(controls_group_);

    // Add description
    auto description = new QLabel(
        "Monitor real-time performance metrics including CPU usage, memory "
        "consumption, "
        "and frame rate. This demonstrates the DeclarativeUI performance "
...
void PerformanceMonitor::updateMetrics() {
    ...
    // Simulate performance metrics (in a real implementation, these would be
    ...
    if (counter % 5 == 0) {
        metrics_log_->append(
            QString("[%1] CPU: %2%, Memory: %3MB, FPS: %4")
                .arg(QDateTime::currentDateTime().toString("hh:mm:ss"))
                .arg(cpu_usage)
                .arg(memory_usage)
                .arg(fps));
