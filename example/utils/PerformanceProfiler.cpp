...
PerformanceProfiler::~PerformanceProfiler() { stopProfiling(); }
...
void PerformanceProfiler::startProfiling() {
    ...
    if (auto_memory_monitoring_) {
        ...
        connect(system_timer_, &QTimer::timeout, this,
                &PerformanceProfiler::updateSystemMetrics);
        system_timer_->start(update_interval_);
    }

    recordMetric("profiling_started", QDateTime::currentMSecsSinceEpoch(),
                 "timestamp");
...
void PerformanceProfiler::stopProfiling() {
    ...
    recordMetric("profiling_stopped", QDateTime::currentMSecsSinceEpoch(),
                 "timestamp");
...
void PerformanceProfiler::pauseProfiling() {
    if (!profiling_active_ || profiling_paused_)
        return;
...
void PerformanceProfiler::recordMetric(const QString& name, qint64 value,
                                       const QString& unit) {
    if (!profiling_active_ || profiling_paused_)
        return;
...
void PerformanceProfiler::recordMemoryUsage(const QString& context) {
    ...
    recordMetric(QString("memory_usage_%1").arg(context), memory_usage,
                 "bytes");
}
...
void PerformanceProfiler::recordCustomMetric(const QString& name,
    ...
    if (value.type() == QVariant::Int || value.type() == QVariant::LongLong) {
        ...
        numeric_value = static_cast<qint64>(
            value.toDouble() *
            1000);  // Convert to integer with 3 decimal precision
    } else {
        numeric_value =
            value.toString().length();  // Use string length as fallback
    }
...
QList<PerformanceMetric> PerformanceProfiler::getMetrics(
...
QString PerformanceProfiler::generateReport() const {
    ...
    for (const QString& category : categories) {
        ...
        stream << "Category: " << category << " (" << category_metrics.size()
               << " metrics)\n";

        // Show recent metrics for this category
        int shown = 0;
        for (auto it = category_metrics.rbegin();
             it != category_metrics.rend() && shown < 5; ++it, ++shown) {
...
qint64 PerformanceProfiler::getCurrentMemoryUsage() const {
    ...
    return QRandomGenerator::global()->bounded(50, 500) * 1024 *
           1024;  // 50-500 MB
}
...
void PerformanceProfiler::cleanupOldMetrics() {
    ...
    for (auto& pair : metric_history_) {
        ...
        if (history.size() >
            max_metrics_ / 10) {  // Keep 10% of max for history
