...
void StatusIndicator::setupUI() {
    ...
    connect(auto_hide_timer_, &QTimer::timeout, this,
            &StatusIndicator::onAutoHideTimeout);

    animation_timer_ = new QTimer(this);
    connect(animation_timer_, &QTimer::timeout, this,
            &StatusIndicator::updateAnimation);
...
void StatusIndicator::updateAppearance() {
    ...
    if (show_message_) {
        ...
        message_label_->setStyleSheet(
            QString("QLabel { color: %1; }").arg(status_color.name()));
    }
    ...
    setStyleSheet(QString("StatusIndicator {"
                          ...
                          "}")
                      .arg(status_color.name())
                      .arg(status_color.darker(120).name()));
}
...
void StatusIndicator::onAutoHideTimeout() { clearStatus(); }
    ...
    if (current_status_ == StatusType::Loading) {
        ...
        icon_label_->setStyleSheet(
            QString("QLabel {"
                    ...
                    "}")
                .arg(getStatusColor(current_status_).name())
                .arg(rotation));
    }
}
