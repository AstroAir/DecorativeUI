...
void DataTable::setupUI() {
    ...
    // Connect signals
    connect(table_, &QTableWidget::cellClicked, this, &DataTable::onItemClicked);
...
void DataTable::enableFiltering(bool enabled) {
...
void DataTable::onPageChanged() {
    updateTable();
}

void DataTable::onFilterChanged() {
    updateTable();
}
...
void DataTable::updateTable() {
    ...
    for (int i = start_row; i < end_row; ++i) {
        ...
        for (int j = 0; j < row_data.size() && j < table_->columnCount(); ++j) {
            table_->setItem(i - start_row, j, new QTableWidgetItem(row_data[j]));
        }
    }
...
void DataTable::updatePagination() {
    ...
    page_label_->setText(QString("Page %1 of %2").arg(current_page_ + 1).arg(total_pages));
}
