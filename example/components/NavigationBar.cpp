...
void NavigationBar::setupUI() {
    setFixedHeight(50);
    setStyleSheet(QString("NavigationBar { background-color: %1; border-bottom: 1px solid #bdc3c7; }")
    ...
    highlight_indicator_->setStyleSheet(QString("background-color: %1;").arg(highlight_color_.name()));
    highlight_indicator_->hide();
    
    // Setup animation
    if (animation_enabled_) {
        highlight_animation_ = std::make_unique<QPropertyAnimation>(highlight_indicator_, "geometry");
        highlight_animation_->setDuration(300);
        highlight_animation_->setEasingCurve(QEasingCurve::OutCubic);
    }
}

void NavigationBar::addItem(const QString& name, const QString& icon, const QString& tooltip) {
...
void NavigationBar::setCurrentIndex(int index) {
    ...
    if (animation_enabled_ && old_index >= 0) {
        ...
        if (index < static_cast<int>(buttons_.size())) {
            auto button = buttons_[index];
            highlight_indicator_->setGeometry(
                button->x(), height() - 3, button->width(), 3
            );
...
void NavigationBar::setHighlightColor(const QColor& color) {
    ...
    highlight_indicator_->setStyleSheet(QString("background-color: %1;").arg(color.name()));
}
...
void NavigationBar::createNavigationButton(const NavigationItem& item, int index) {
...
void NavigationBar::animateToItem(int index) {
    if (!animation_enabled_ || !highlight_animation_) return;
    if (index < 0 || index >= static_cast<int>(buttons_.size())) return;
    
    auto target_button = buttons_[index];
    QRect target_rect(target_button->x(), height() - 3, target_button->width(), 3);
